...
class BrandSeeAllController extends GetxController {
  ...
  RxList imageCategoryList = [].obs;
  bool isSearch = false;

  getImageCategoryList(
      context, currentPage, String imageCategoryTypeId, bool hideloading,
    ...
    try {
      ...
      var pageURL = '${ApiUrl.getImageCategoryList}?page=$currentPage';
      ...
      if (response.statusCode == 200) {
        if (responseData['success'] == true) {
          ...
          message.value = '';
          var categoryData = ImageCategoryModel.fromJson(responseData);
          if (isFirstTime == true && imageCategoryList.isNotEmpty) {
            currentPage = 1;
            imageCategoryList.clear();
          }
          if (categoryData.data.data.isNotEmpty) {
            imageCategoryList.addAll(categoryData.data.data);
            imageCategoryList.refresh();
            update();
          } else {
            imageCategoryList.clear();
          }
  ...
  getListItem(BuildContext context, String categoryId,
      {required ImageCategoryData data}) {
    return GestureDetector(
      onTap: () {
        Get.to(BrandImageScreen(
                id: data.imageCategoryTypeId.toString(), title: data.name))!
            .then((value) {});
      },
      child: Container(
        height: 15.h,
